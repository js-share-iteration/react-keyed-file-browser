...
class RawFileBrowser extends React.Component {
  ...
  endAction = () => {
    if (
      ...
      this.state.selection.indexOf('__new__') !== -1
    ) {
      ...
      if (selectedItem.action) {
        ...
        actions = (
          ...
          <div className="item-actions">
            <i className="icon loading fa fa-circle-o-notch fa-spin" />{' '}
            {actionText}
          </div>
        );
      } else {
        ...
        if (
          ...
          ((selectionIsFolder &&
            typeof this.props.onRenameFile === 'function') ||
            (!selectionIsFolder &&
              typeof this.props.onRenameFolder === 'function'))
        ) {
          actions.push(
